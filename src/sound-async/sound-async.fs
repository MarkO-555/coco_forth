...
: sq1   ( -- )  snd-square 880 0 14  0 1tone ;
: sq2   ( -- )  snd-square 440 0 14  0 1tone ;
: sq3   ( -- )  snd-square 220 0 14  0 1tone ;
: sq4   ( -- )  snd-square 110 0 14  0 1tone ;
: swp   ( -- )  snd-wave  1500 0 12 -600 1tone ;     \ descending sweep
: saw1  ( -- )  snd-saw    220 0 16  0 1tone ;
: saw2  ( -- )  snd-saw    440 0 16  0 1tone ;
: tri8  ( -- )  snd-tri    330 0 16  0 1tone ;
: sin9  ( -- )  snd-wave   440 0 18  0 1tone ;
: beep  ( -- )  snd-square 700 0  5  0 1tone ;        \ short blip
: zap   ( -- )  snd-saw   2800 0  6 -1700 1tone ;    \ maser: high saw, fast fall
: hit   ( -- )  snd-square 400 0  5 -300 1tone ;
: rise  ( -- )  snd-wave   300 0 14  500 1tone ;

\ Two-tone dock chime.
: dock  ( -- )
  seq0
  snd-square 600 0 10 0 nq-add
  snd-square 300 0 12 0 nq-add ;

\ Three sine blips with silent rests between (rest = freq 0, amp 8).
: chirp  ( -- )
  seq0
  snd-wave 1400 0 2 0 nq-add
  snd-wave    0 8 2 0 nq-add
  snd-wave 1400 0 2 0 nq-add
  snd-wave    0 8 2 0 nq-add
  snd-wave 1400 0 2 0 nq-add ;
