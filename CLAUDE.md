...
The project includes a **Forth kernel and cross-compiler** (the project root), a growing collection of **SG4 semigraphics demos** (`src/`), and a **tutorial series** (`getting-started/`) that walks through building programs from scratch. The long-term vision lives in `COCO_RENOVATION.md`.
...
Official documentation is HTML (`tutorials/`, `reference.html`), not markdown. Markdown files are working notes for orientation.

- **`tutorials/index.html`** — Getting Started tutorial (13 chapters), table of contents, entry point
- **`reference.html`** — Language reference: kernel primitives, compiler directives, library words, memory map
- **`tutorials/00-about-forth/index.html` through `tutorials/13-onto-your-coco/index.html`** — Tutorial chapters (each in its own dir with `images/` and `examples/`)

Markdown working notes:
- **`kernel/README.md`** — Kernel architecture, primitives, memory map, 6809 gotchas
- **`tools/README.md`** — fc.py compiler pipeline and CODE word syntax
...
- Always build BOTH kernels after changing `kernel/kernel.asm`: run `make` (ROM mode) AND `make allram` in `kernel/`. The default `make` only rebuilds the ROM-mode kernel; downstream all-RAM demos (e.g. spacewarp) link against `build/kernel-allram.bin` and will silently use the stale variant if it isn't rebuilt.
...
Primary doc: `COCO_RENOVATION.md`. Tech reference: `coco-guides/coco_technical_reference.pdf`.
...
## Key Files
- `kernel/kernel.asm` — 6809 Forth executor, assembles with lwasm
- `kernel/Makefile` — `make` builds, `make run` launches XRoar
- `tools/fc.py` — Forth cross-compiler (Forth source → DECB binary)
- `src/hello/hello.fs` — Hello World source
...
Source of truth: kernel.asm KEY_TABLE (line ~763), NOT lib/keyboard.fs.
